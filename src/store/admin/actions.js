...
const getActiveAuthors = createAsyncThunk(
  "getAuthor", 
  async (token) => {
  try {
    let headers = { headers: { Authorization: `Bearer ${token}` } };
    const response = await axios.get(
      `http://localhost:8000/api/authors?active=true`,
      headers
    );
    return {
...
const getActiveCompanies = createAsyncThunk(
  "getCompanies", 
  async (token) => {
  try {
    let headers = { headers: { Authorization: `Bearer ${token}` } };
    const response = await axios.get(
      `http://localhost:8000/api/companies?active=true`,
      headers
    );
    return {
...
const UpdateUserToAuthor = createAsyncThunk(
  'updateRoleAuthor', async(id, token)=>{
    try {
...
const UpdateUserToCompany = createAsyncThunk(
  'updateRoleCompany', async(id, token)=>{
    try {
